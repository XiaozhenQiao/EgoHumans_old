...
def merge_temporal_data(root_path, output_path, scale_factor=2.0, invalid_arias=None, invalid_exos=None, manual_exo_cameras=None, max_frames=None, logger=None):
    """合并时序数据，包含去畸变和缩放处理"""
    if logger is None:
        logger = logging.getLogger(__name__)
    ...
    logger.info(f"  - 标志位命名规则: has_kp3d, has_betas/has_body_pose/has_global_orient/has_transl, has_kp2d_{{cam}}, has_bbox_{{cam}}")
